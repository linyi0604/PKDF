...
class Config(object):
    ...
    def __init__(self, cross_val_index):
        ...
        self.model_save_path = "./result/model_state/student/model_%s"%(cross_val_index)+".pkl"
        self.teacher_model_load_path = "./result/model_state/teacher/model_"+cross_val_index+".pkl"
        self.log_file = "./logs/student/%s"%(cross_val_index)+".log"
        self.num_workers = 16
        self.resize = (240, 240)
...
def train_test(cross_val_index):
    ...
    writer = SummaryWriter(log_dir=config.log_file.rstrip(".log"))
    log = Logger()
    ...
    dset_train = dataset_processing.DatasetProcessing_teacher_student_augmentation(
        ...
        transform_image=transforms.Compose([
                ...
                transforms.Scale(config.resize),
    ...
    for epoch in range(config.epoch):
        ...
        log.write(message)
        writer.add_scalar("train cls loss", losses_cls.avg, global_step=epoch)
        writer.add_scalar("train distr loss", losses_distribution.avg, global_step=epoch)
        ...
        with torch.no_grad():
            ...
            pre_se_sp_yi_report = str(pre_se_sp_yi_report) + \
                                  "\n cls_loss: %.4f, distribution_loss:%.4f" \
                                  % (losses_cls.avg, losses_distribution.avg)

            writer.add_scalar("test cls loss", losses_cls.avg, global_step=epoch)
            writer.add_scalar("test distr loss", losses_distribution.avg, global_step=epoch)
